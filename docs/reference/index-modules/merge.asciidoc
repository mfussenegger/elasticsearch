...
The maximum merge backlog (running or pending merges) after which index throttling
kicks in.  This setting must be at least as high as max_thread_count.  Index throttling
will restrict indexing to one thread to allow the merges backlog to catch up.
